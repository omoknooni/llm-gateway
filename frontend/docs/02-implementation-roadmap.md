...
저장소 전체 계획의 **Phase 3(admin 콘솔)** 이 이 문서의 범위이고, Phase 4의 관측 화면은
backend M6~M8이 열린 뒤입니다.
...
| F4 모델 카탈로그·허용 모델 | 완료 |
| F5 설정·내 정보·대시보드 | 완료 |
| F6 관측 화면 | 미착수 (backend M6~M8 대기) |

**검증 범위**: 타입 검사(`tsc --noEmit`), lint, 프로덕션 빌드, 단위 테스트 33건이 통과합니다.
로그인 → 세션 쿠키 → 리다이렉트 경로와 Admin API 다운 시 에러 경계 동작은 실행 중인 서버에
직접 요청을 보내 확인했습니다.

**미검증 항목**: 개발 환경에 Admin API 인스턴스가 없어 **실제 응답에 대고 돌린 화면별 검증은
아직 없습니다.** 목록·상세·변경 조작이 실제 스키마와 맞는지는 backend를 띄운 뒤 확인해야
합니다. backend 쪽도 PostgreSQL 미적용 상태라(backend 07 문서) 통합 확인은 DB가 서는 시점에
함께 진행합니다.
...
### F6 — 관측 화면 (backend M6~M8 대기)

예산, 사용량·비용 대시보드, 리더보드, rate limit. **backend가 API를 내기 전에는 착수하지
않습니다.** 차트 라이브러리 선정도 이 시점의 결정입니다(미결정 #2).
...
- **금액 문자열이 숫자로 변환되지 않는 것** — 표시 포맷을 거쳐도 정밀도가 유지되어야 합니다.
...
| 2 | 차트 라이브러리 선정 | F6 | 대시보드 지표가 확정된 뒤 |
