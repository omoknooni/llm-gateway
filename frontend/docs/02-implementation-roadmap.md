...
**미검증 항목**: F6 화면들은 **실제 응답에 대고 돌린 검증이 아직 없습니다.** backend가 로컬
PostgreSQL로 통합 검증을 마쳤으므로(backend 07 문서), 다음 단계는 Admin API를 띄워 두고 화면별로
목록·상세·변경 조작을 실제 스키마와 대조하는 것입니다. 특히 집계 job이 돌기 전에는 사용량·
breakdown이 비어 있는 것이 정상이라, **빈 화면과 잘못된 화면을 구분하려면 시드 데이터가
필요합니다.**
