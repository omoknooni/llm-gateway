...
배분 불변식은 "사용자 예산 합계 ≤ 팀 예산"입니다. 이 검증은 **읽은 행과 쓰는 행이 다릅니다**
— 팀 한도와 기존 배분을 읽고, 그와 다른 사용자의 `budget_configs` 행을 넣습니다. 그래서 행
잠금으로는 막을 수 없는 write skew 가 생깁니다.

```text
팀 한도 100, 배분 없음
  요청 A: 사용자 X 에 60  → 합계 0 을 읽음 → 60 ≤ 100 통과 → X 행 INSERT
  요청 B: 사용자 Y 에 60  → 합계 0 을 읽음 → 60 ≤ 100 통과 → Y 행 INSERT
  두 요청은 서로 다른 행을 넣어 충돌하지 않고 둘 다 커밋 → 최종 합계 120
```

팀 예산이 아직 없는 상태에서는 잠글 행 자체가 없다는 문제도 있습니다.

**팀 id 를 키로 한 트랜잭션 스코프 advisory lock**(`pg_advisory_xact_lock`)으로 같은 팀의
예산 쓰기를 직렬화합니다. 커밋·롤백에 자동으로 풀리므로 해제를 잊을 수 없습니다.

| 경로 | 잠금 |
|---|---|
| 팀 예산 설정 | 필요 — 한도를 배분 합계보다 낮추는 것을 막습니다 |
| 사용자 예산 설정 | 필요 — 소속 팀을 잠급니다 |
| 배분 일괄 설정 | 필요 |
| 팀/사용자 예산 해제 | **불필요** — 제약을 없애거나 합계를 줄이는 방향이라 불변식을 깰 수 없습니다 |
...
| 금액 자릿수 | 입력에 `numeric(14,4)` 와 **같은 제약**(`max_digits=14`, `decimal_places=4`). 없으면 `0.00005` 가 DB 에서는 반올림되고 카운터 문자열에서는 잘려 두 값이 갈립니다 |
| 예산 쓰기 직렬화 | 팀 id 기반 **트랜잭션 advisory lock**. 아래 참조 |
| 재시드 실패 | 카운터 갱신에 실패하면 **전체 취소 + 503** `counter_write_failed`. 부분 적용 없음. 아래 참조 |
